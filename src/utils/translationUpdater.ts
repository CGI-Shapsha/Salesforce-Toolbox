...
export class TranslationUpdater {
    ...
    public static async doUpdate(options: UpdaterOptionsType): Promise<void> {
        ...
        const activatedLanguages = (await utils.getMetadataList(options.connection, registry.types.translations.name)).map(l => l.fullName);
        ...
        if (isObjectTranslations) {
            ...
            await translationsHelper.updateObjectTranslationsFile(options, config, existingTranslations, retrievedTranslations, activatedLanguages);
            options.rootClass.spinner.stop('✔️');
        }
        ...
        if (isTranslations) {
            ...
            await translationsHelper.updateTranslations(options, config, existingTranslations, retrievedTranslations, activatedLanguages);
            options.rootClass.spinner.stop('✔️');
        }
