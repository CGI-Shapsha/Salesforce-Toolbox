/**
 * @description main process for profile update
...
export class ProfileUpdater {
  ...
  public static async doUpdate(options: UpdaterOptionsType): Promise<void> {
    ...
    for (const profileInfo of localProfilesInfo) {
      ...
      const correspondingRetreivedProfileInfo = retreivedProfilesInfo.find(
        (pi) => JSON.stringify(pi.filename) === JSON.stringify(profileInfo.filename)
      );
      ...
      if (config.isFieldPermissions) {
        ...
        currentProfile = permsHelper.updateFieldPermissions(currentProfile, updatedProfile, fieldsPermissionsToUpdate);
      }
      ...
      if (config.isLayoutAssignments) {
        ...
        currentProfile = permsHelper.updateLayoutPermissions(currentProfile, updatedProfile, layoutPermissionsToUpdate);
      }
      ...
      if (config.isApexPermissions) {
        ...
        currentProfile = permsHelper.updateApexPermissions(currentProfile, updatedProfile, apexPermissionsToUpdate);
      }
      ...
      if (config.isPagePermissions) {
        ...
        currentProfile = permsHelper.updatePagePermissions(currentProfile, updatedProfile, pagePermissionsToUpdate);
      }

      if (config.isCustomApplicationPermissions) {
        const customAppPermissionsToUpdate = permsHelper.getAllGenericPermissionsToUpdate(
      ...
      if (config.isCustomMetadataPermissions) {
        const customMDTToUpdate = permsHelper.getAllGenericPermissionsToUpdate(
          config.customMetadataTypes,
          'custMetadata'
        );
        currentProfile = permsHelper.updateCustomMetadataPermissions(currentProfile, updatedProfile, customMDTToUpdate);
      }

      if (config.isCustomPermissionPermissions) {
        const customPermPermissionsToUpdate = permsHelper.getAllGenericPermissionsToUpdate(
      ...
      if (config.isCustomSettingPermissions) {
        const customSettingPermissionsToUpdate = permsHelper.getAllGenericPermissionsToUpdate(
      ...
      if (config.isCustomTabPermissions) {
        const customTabsPermissionsToUpdate = permsHelper.getAllGenericPermissionsToUpdate(
          config.customTabs,
          'custTab'
        );
      ...
      if (config.isUserPermissions) {
        const userPermissionsToUpdate = permsHelper.getAllGenericPermissionsToUpdate(
          config.userPermissions,
          'userPerm'
        );
        currentProfile = permsHelper.updateUserPermissions(currentProfile, updatedProfile, userPermissionsToUpdate);
      }

      if (config.isExternalDataSource) {
        const externalDataSourceToUpdate = permsHelper.getAllGenericPermissionsToUpdate(
      ...
      if (config.loginIpRanges) {
        currentProfile = permsHelper.updateLoginIpRanges(currentProfile, updatedProfile);
      }

      if (config.loginHours) {
        currentProfile = permsHelper.updateLoginHours(currentProfile, updatedProfile);
      }
