/**
 * @description Methods used to update permissions
...
const updateFieldPermissions = function (
  ...
  allCurrentFieldPermissions =
    allCurrentFieldPermissions?.filter(
      (fp) => !JSON.stringify(fieldsPermissionsToUpdate).includes(JSON.stringify(fp.field))
    ) ?? [];
  ...
  if (allFieldSetForObj && allFieldSetForObj.length > 0) {
    for (const allF of allFieldSetForObj) {
      allCurrentFieldPermissions = allCurrentFieldPermissions.filter(
        (fp) => !fp.field[0].startsWith(allF[0].split('*')[0])
      );
    }
  }
...
const updateObjectPermissions = function (
  ...
  allCurrentObjectPermissions =
    allCurrentObjectPermissions?.filter(
      (op) => !JSON.stringify(objectsPermissionsToUpdate).includes(JSON.stringify(op.object))
    ) ?? [];
...
const updateRecordTypePermissions = function (
  ...
  for (const objToUpdateRT of recordTypePermissionsToUpdate) {
    allCurrentRecordTypePermissions =
      allCurrentRecordTypePermissions?.filter((rtp) => !rtp.recordType[0].startsWith(`${objToUpdateRT[0]}.`)) ?? [];
  }
...
const updateLayoutPermissions = function (
  ...
  for (const objToUpdateLA of layoutPermissionsToUpdate) {
    allCurrentLayoutPermissions =
      allCurrentLayoutPermissions?.filter((lp) => !lp.layout[0].startsWith(objToUpdateLA[0])) ?? [];
  }
...
const updateApexPermissions = function (
  ...
  allCurrentApexPermissions =
    allCurrentApexPermissions?.filter(
      (ap) => !JSON.stringify(apexPermissionsToUpdate).includes(JSON.stringify(ap.apexClass))
    ) ?? [];

  // add new ObjectPermissions
  for (const apexPermission of apexPermissionsToUpdate) {
    const newAP = allUpdatedApexPermissions?.find(
      (ap) => JSON.stringify(apexPermission) === JSON.stringify(ap.apexClass)
    );
    if (!newAP) continue;
...
const updatePagePermissions = function (
  ...
  allCurrentPagePermissions =
    allCurrentPagePermissions?.filter(
      (pp) => !JSON.stringify(pagePermissionsToUpdate).includes(JSON.stringify(pp.apexPage))
    ) ?? [];

  // add new ObjectPermissions
  for (const pagePermission of pagePermissionsToUpdate) {
    const newPP = allUpdatedPagePermissions?.find(
      (pp) => JSON.stringify(pagePermission) === JSON.stringify(pp.apexPage)
    );
    if (!newPP) continue;
...
const updateCustomAppPermissions = function (
  ...
  allCurrentCustomAppPermissions =
    allCurrentCustomAppPermissions?.filter(
      (ap) => !JSON.stringify(customAppPermissionsToUpdate).includes(JSON.stringify(ap.application))
    ) ?? [];

  // add new applicationVisibilities
  for (const customAppPermission of customAppPermissionsToUpdate) {
    const newAP = allUpdatedCustomAppPermissions?.find(
      (ap) => JSON.stringify(customAppPermission) === JSON.stringify(ap.application)
    );
    if (!newAP) continue;
...
const updateCustomMetadataPermissions = function (
  ...
  allCurrentMDTPermissions =
    allCurrentMDTPermissions?.filter(
      (mdtp) => !JSON.stringify(customMDTToUpdate).includes(JSON.stringify(mdtp.name))
    ) ?? [];
...
const updateCustomPermPermissions = function (
  ...
  allCurrentCustomPermPermissions =
    allCurrentCustomPermPermissions?.filter(
      (cpp) => !JSON.stringify(customPermPermissionsToUpdate).includes(JSON.stringify(cpp.name))
    ) ?? [];

  // add new customPermissions
  for (const customPermPermission of customPermPermissionsToUpdate) {
    const newCPP = allUpdatedCustomPermPermissions?.find(
      (cpp) => JSON.stringify(customPermPermission) === JSON.stringify(cpp.name)
    );
    if (!newCPP) continue;
...
const updateCustomSettingPermissions = function (
  ...
  allCurrentCustomSettingPermissions =
    allCurrentCustomSettingPermissions?.filter(
      (csp) => !JSON.stringify(customSettingPermissionsToUpdate).includes(JSON.stringify(csp.name))
    ) ?? [];

  // add new customSettingAccesses
  for (const customSettingPermission of customSettingPermissionsToUpdate) {
    const newCSP = allUpdatedCustomSettingPermissions?.find(
      (csp) => JSON.stringify(customSettingPermission) === JSON.stringify(csp.name)
    );
    if (!newCSP) continue;
...
const updateCustomTabPermissions = function (
  ...
  allCurrentCustomTabPermissions =
    allCurrentCustomTabPermissions?.filter(
      (ctp) => !JSON.stringify(customTabsPermissionsToUpdate).includes(JSON.stringify(ctp.tab))
    ) ?? [];

  // add new tabVisibilities
  for (const customTabPermission of customTabsPermissionsToUpdate) {
    const newCTP = allUpdatedCustomTabPermissions?.find(
      (ctp) => JSON.stringify(customTabPermission) === JSON.stringify(ctp.tab)
    );
    if (!newCTP) continue;
...
const updateUserPermissions = function (
  ...
  allCurrentUserPermPermissions =
    allCurrentUserPermPermissions?.filter(
      (up) => !JSON.stringify(userPermissionsToUpdate).includes(JSON.stringify(up.name))
    ) ?? [];

  // add new userPermissions
  for (const userPermPermission of userPermissionsToUpdate) {
    const newUP = allUpdatedUserPermPermissions?.find(
      (up) => JSON.stringify(userPermPermission) === JSON.stringify(up.name)
    );
    if (!newUP) continue;
...
const updateExtDataSourcePermissions = function (
  ...
  allCurrentExtDataSourcePermissions =
    allCurrentExtDataSourcePermissions?.filter(
      (edsp) => !JSON.stringify(externalDataSourceToUpdate).includes(JSON.stringify(edsp.externalDataSource))
    ) ?? [];

  // add new userPermissions
  for (const extDataSourcePermission of externalDataSourceToUpdate) {
    const newEDSP = allUpdatedExtDataSourcePermissions?.find(
      (edsp) => JSON.stringify(extDataSourcePermission) === JSON.stringify(edsp.externalDataSource)
    );
    if (!newEDSP) continue;
...
const updateLoginIpRanges = function (currentProfile: ProfileCustom, updatedProfile: ProfileCustom): ProfileCustom {
...
const updateLoginHours = function (currentProfile: ProfileCustom, updatedProfile: ProfileCustom): ProfileCustom {
